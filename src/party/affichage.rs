...
pub fn affichage(robot: &mut Vec<R>, terrain: (i32, i32), obstacle: &mut Vec<OB>) {
    ...
    for _i in 0..=terrain.0 {
        //on remplit toute notre grille avec des points
    ...
    println!("{}", "");
    for v in &le_vecteur {
        //on parcourt la grille pour l'afficher
        compteur_colonne = compteur_colonne + 1; //on change de colonne
        if compteur_ligne < 10 {
            //tant qu'on a un nombre de ligne inférieur à 10 l'espace entre les numéros des
            print!("{}", " ");
        }
